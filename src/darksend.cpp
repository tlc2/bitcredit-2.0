...
void CDarksendPool::ProcessMessageDarksend(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "dsa") { //Darksend Accept Into Pool
        ...
        if(sessionUsers == 0) {
            if(pmn->nLastDsq != 0 &&
                pmn->nLastDsq + mnodeman.CountBasenodesAboveProtocol(MIN_PEER_PROTO_VERSION)/5 > mnodeman.nDsqCount){
...
bool CDarksendPool::DoAutomaticDenominating(bool fDryRun, bool ready)
{
    ...
    int64_t nBalanceNeedsAnonymized = nAnonymizeBitcreditAmount*COIN - pwalletMain->GetAnonymizedBalance();
    ...
    if(!sessionFoundBasenode){
        ...
        double fBitcreditSubmitted = (sessionTotalValue / CENT);
        LogPrintf("Submitting Darksend for %f DRK CENT - sessionTotalValue %d\n", fBitcreditSubmitted, sessionTotalValue);
