	...
	for (unsigned int i = 0; i < block.vtx[0].vout.size(); i++){
		...
		if (block.vtx[0].vout[i].scriptPubKey == BANK_SCRIPT){
			...
			if (bank_subsidy < bankfund)
				return state.DoS(100, error("ConnectBlock() : coinbase does not yeild enough to the bank (actual=%d vs required=%d)", bank_subsidy, bankfund));
			}
	}
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    //for (int32_t version = 1; version < 2; ++version) // check for version 2, 3 and 4 upgrades
    // Reject outdated version blocks when 95% (75% on testnet) of the network has upgraded:
    int32_t version = 1;
        if (block.nVersion != version && IsSuperMajority(version, pindexPrev, consensusParams.nMajorityRejectBlockOutdated, consensusParams))
            return state.Invalid(false, REJECT_OBSOLETE, strprintf("bad-version(v%d)", version),
                                 strprintf("rejected nVersion=%d block", version));
